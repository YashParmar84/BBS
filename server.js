...
const app = express();
const PORT = process.env.PORT || 3000; // Fixed PORT
...
app.post('/api/login', (req, res) => {
    try {
        ...
        return res.status(401).json({ success: false, message: 'Invalid credentials or access code' });
...
app.post('/api/toggle-item', (req, res) => {
    try {
        ...
        if (user) {
            ...
            if (index > -1) {
                user.itemsFound[taskId].splice(index, 1);
            } else {
                user.itemsFound[taskId].push(itemIndex);
            }
...
app.get('/api/admin/data', (req, res) => res.json(readData()));
...
app.post('/api/admin/toggle-user-item', (req, res) => {
    try {
        ...
        console.error("Toggle user item error:", error);
        res.status(500).json({ success: false });
    }
});
...
app.post('/api/admin/toggle-disqualification', (req, res) => {
    try {
        ...
        if (user) {
            ...
            if (!disqualified) user.wrongAttempts = {};
            writeData(data);
            return res.json({ success: true });
        }
        res.status(404).json({ success: false });
    } catch (error) {
        console.error("Toggle disqualification error:", error);
        res.status(500).json({ success: false });
    }
});
...
// Fallback routes for logo and background images
app.get('/logo.jpg', (req, res) => res.sendFile(path.join(__dirname, 'technomatra logo.jpeg')));
app.get('/bg.jpg', (req, res) => res.sendFile(path.join(__dirname, 'technomatra logo.jpeg')));
